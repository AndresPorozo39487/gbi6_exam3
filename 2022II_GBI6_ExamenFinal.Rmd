...
**El primer gráfico ( p1) es un gráfico de barras de error agrupadas con puntos con fluctuaciones. Muestra la relación entre la longitud del pico y el sexo de tres especies diferentes de pingüinos. El eje x representa la especie, el eje y representa la longitud del pico y el color representa el sexo. La posición de los puntos está alterada para una mejor visibilidad,p2 es un gráfico de cajas con puntos con fluctuaciones que muestra la relación entre el ancho del pico y la especie de pingüino. El eje x representa la especie, el eje y representa el ancho del pico y el color representa la especie.

p3 es un gráfico de dispersión con línea de regresión que muestra la relación entre la longitud y el ancho del pico para todas las especies de pingüinos. El color representa la especie y se utiliza una escala logarítmica en ambos ejes **
...
p4 <- ggplot(penguins, aes(x = body_mass_g, y = flipper_length_mm, color = sex)) +
  ...
  labs(x = "Masa corporal (g)", y = "Longitud de aleta (mm)") +
  ...
  stat_poly_eq(aes(label = paste(..eq.label.., ..rr.label.., sep = "~~~~")),
               ...
               label.x = "right",
               label.y = "top")
p5 <- ggplot(penguins, aes(x = bill_length_mm, y = bill_depth_mm)) +
  geom_point(aes(color = body_mass_g, size = flipper_length_mm)) +
  scale_color_gradient(low = "grey", high = "red") +
  scale_size_continuous(range = c(1, 8)) +
  labs(x = "Longitud de pico (mm)", y = "Ancho de pico (mm)") +
  theme_classic() +
  theme(legend.position = "bottom") +
...
gse <- getGEO("GSE120710")
exp <- exprs(gse[[1]])

exp[is.na(exp)] <- 0
exp <- log2(exp + 1)

dist_mat <- dist(exp, method = "euclidean")
hc_rows <- hclust(dist_mat, method = "ward.D2")
hc_cols <- hclust(dist(t(exp)), method = "ward.D2")
```
